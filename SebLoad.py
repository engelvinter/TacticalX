...
class SebLoad:
    ...
    def _adjust_fund_remake(self,df):
        ...
        diff = df.id.diff()
        diff = diff.dropna()
        fund_changes = diff[diff != 0]
        for index, row in fund_changes.iteritems():
            change = df.loc[index].change
            #print(index, change)
            updated_rows = df[index:].quote / (1 + change)
            df.update(updated_rows)

    def _adjust_fund_abnormal(self, df):
        abnormal_change = df[df.change < -0.1]
        for index, row in abnormal_change.iterrows():
            #print(index, row)
            updated_rows = df[index:].quote / (1 + row.change)
            df.update(updated_rows)
